...
var accounts = []
let provider = ethers.providers.getDefaultProvider("any");
let contract;
let userAccount;
let totalPrice;
let salesOpen;
...
let getPrice = async () => {
	quantity = parseInt(document.getElementById(SELECTORS.PINEAPPLES.QUANTITY).value, 10);
...
let mintPineapple = async () => {
	...
	if (canMint) {
		...
		return pineapplesMinter.mintPineapple(quantity, transactionParameters);
	}
	return {
...
let startApp = async () => {
	...
	// 	getSalesState().catch(error => {
	// 		console.log("ERROR getting sales state in loop:");
	// 		console.log(error);
	// 	});
	// }, 1000);
}
